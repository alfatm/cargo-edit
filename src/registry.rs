...
struct Registry {
    index: Option<String>,
    token: Option<String>,
    // `credential-provider` may be a string or an array of strings per the
    // cargo config spec (e.g. `["cargo:token"]` or
    // `["cargo:token-from-stdout", "cmd", "arg"]`). Normalize both to a single
    // space-joined string.
    #[serde(rename = "credential-provider", default, deserialize_with = "de_credential_provider")]
    credential_provider: Option<String>,
}

fn de_credential_provider<'de, D>(deserializer: D) -> Result<Option<String>, D::Error>
where
    D: serde::Deserializer<'de>,
{
    ...
    Ok(match Option::<StringOrVec>::deserialize(deserializer)? {
        ...
        Some(StringOrVec::Vec(v)) => Some(v.join(" ")),
    })
}
